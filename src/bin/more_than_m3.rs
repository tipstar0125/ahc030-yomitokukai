...
fn main() {
    ...
    // 盤面の尤度再計算用メモ
    let mut likelihoods_memo = vec![];
    let mut fortune_coords_memo = vec![];

    for t in 0.. {
        if t == 0 {
            pool = make_board_random(&input, &mut rng, ITER, &mut set); // 盤面候補の生成
        } else {
            // 尤度上位10ケから上下左右に動かす(ポリオミノが2つの場合、これだけでいけちゃう)
            for _ in 0..ITER {
                let good_board = pool[rng.gen_range(0..5)].clone();
                ...
                if set.contains(&next_board.oil_cnt) {
                    continue;
                }
                set.insert(next_board.oil_cnt.clone());
                // 本当は、pool内の尤度を対数尤度にした後、next boardの対数尤度を更新する必要がある
                // 占い山登り時に対数尤度を尤度に戻す
                pool.push(next_board);
            }
        ...
        eprintln!("Board candidate num: {}", pool.len());
        ...
        // 再計算メモ
        likelihoods_memo.push(likelihoods);
        fortune_coords_memo.push(fortune_coords);

        // 都度、最初のクエリから計算
        let L = pool.len() as f64;
        for board in pool.iter_mut() {
            board.prob = (1.0 / L).ln();
            for (likelihoods, fortune_coords) in likelihoods_memo.iter().zip(&fortune_coords_memo) {
                let mut cnt = 0;
                for &coord in fortune_coords.iter() {
                    cnt += board.oil_cnt[coord] as usize;
                }
                // 対数尤度にして、値が小さくなりすぎないようにする
                board.prob += likelihoods[cnt];
            }
        }
        ...
        pool.truncate(10000); // 確率上位10000ケ(適当)のみ残す
    }
...
// ランダムで盤面を生成
fn make_board_random(
    input: &Input,
    rng: &mut rand_pcg::Pcg64Mcg,
    iter: usize,
    set: &mut HashSet<DynamicMap2d<u8>>,
) -> Vec<Board> {
    let mut pool = vec![];
    for _ in 0..iter {
