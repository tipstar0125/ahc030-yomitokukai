...
fn main() {
    ...
    let num_possibilities = (0..input.m)
        ...
        .product::<f64>();
    let capacity =
        1.0 + input.eps * input.eps.log2() + (1.0 - input.eps) * (1.0 - input.eps).log2();
    ...
    for t in 0.. {
        ...
        let info_Q = {
            ...
            let mut size = pool.len().min(
                (100.0 * (160.0 / difficulty).powi(2) * (3.0 - get_time()).min(1.0)).max(2.0)
                    as usize,
            );
            ...
            qs.sort_by_key(|&ij| {
                pool[0].count[ij].min(1) as usize * 1000000 + rng.gen_range(0..1000000)
            });
            qs.reverse();
            let mut crt = -1e100;
            let mut add_k = 0;
        ...
        pool.truncate(
            (100.0 * (160.0 / difficulty).powi(2) * (3.0 - get_time()).min(1.0)).max(1.0) as usize,
        );
    }
...
impl Sim {
        ...
        for k in 1..=input.n * input.n {
            for S in 0..=input.total {
                ...
                for r in (0..=mu.round() as usize).rev() {
                    let prob = probability_in_range(
                        mu,
                        sigma,
                        if r == 0 { -100.0 } else { r as f64 - 0.5 },
                        r as f64 + 0.5,
                    );
                    if prob < EPS {
                        probs_lb[k][S] = r + 1;
    ...
    fn ans(&mut self, T: &Vec<usize>) -> bool {
        ...
        println!(
            ...
            T.len(),
            T.iter()
                .map(|&ij| format!("{} {}", ij / self.n, ij % self.n))
                .join(" ")
        );
