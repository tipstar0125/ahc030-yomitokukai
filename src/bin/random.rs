...
fn main() {
    ...
    loop {
        let k = 20;
        let mut fortune_coords = (0..input.n2)
            .map(|ij| Coord::new(ij / input.n, ij % input.n))
            .collect_vec();
        fortune_coords.shuffle(&mut rng);
        fortune_coords = fortune_coords.into_iter().take(k).collect_vec();
